...
const Horario = ({ userId }) => {
  ...
  const [materiaAdd, setMateriaAdd] = useState();
      ...
      >
        ...
        >
          <Text style={{ fontSize: isSelected ? 15 : 13 }}>
            {isSelected
              ? `NRC: ${item.nrc}`
              : `${getDateFormat24(item.hora_inicio)} - ${getDateFormat24(
                  item.hora_final
                )}`}
          </Text>
          <Text style={{ fontSize: isSelected ? 15 : 13 }}>
  ...
  const renderItem = ({ item, index }) => {
    ...
    const width = item.nrc === selectedId ? "95%" : "90%";
      ...
      <>
        {index !== inscripciones.length ? (
          <Item
            ...
            isSelected={item.nrc === selectedId}
  ...
  const filterMaterias = () => {
    const materiasFiltradas = materias.filter(
      (materia) =>
        !inscripciones.some((inscripcion) => inscripcion.nrc === materia.nrc)
    );
  ...
  const addInscripcion = (addProf = true) => {
    setLoading(true);
    ...
    try {
      const options = {
        ...
        body: JSON.stringify({
          nrc: addProf ? materiaAdd.nrc : selectedId,
          profesor: addProf ? userId : null,
        }),
      };

      fetch(`${process.env.EXPO_PUBLIC_API_URL}/materias/prof_update`, options)
        .then((response) => {
        ...
        .catch((error) => {
          console.log(
            `Fetch error to: ${process.env.EXPO_PUBLIC_API_URL}/materias/prof_update`,
            error
          );
        });
    } catch (error) {}
  ...
  const fetchMaterias = () => {
    ...
    fetch(`${process.env.EXPO_PUBLIC_API_URL}/materias`, options)
      ...
      .then((data) => {
        setMaterias(data.filter((item) => item.profesor === null));
      })
      .catch((error) => {
        console.log(
  ...
  const fetchInscripciones = () => {
    ...
    fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/profesores/horario/${userId}`,
      options
    )
      .then((response) => response.json())
      .then((data) => {
      ...
      .catch((error) => {
        console.log(
          `Fetch error to: ${process.env.EXPO_PUBLIC_API_URL}/profesores/horario/${userId}`,
          error
        );
      })
      .finally(() => {
  ...
  return (
    <View style={stylesLocal.container}>
      {Array.isArray(inscripciones) && inscripciones[0]?.nrc !== 0 && (
        <FlatList
          data={[...inscripciones, { nrc: -1 }]}
      ...
      <Portal>
        ...
        >
          ...
          <SelectDropdown
            ...
            onSelect={(item) => setMateriaAdd(item)}
            search
...
export default function HorarioProfesoresAdmin({ route, navigation }) {
